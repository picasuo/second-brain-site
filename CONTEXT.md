...
**Published Note Sequence Navigation**:
Published Note 窗口 footer 中遍历完整 Publish Set 的相邻笔记导航；它遵循 Published Note Order，上一篇指向更靠前、通常更新的笔记，下一篇指向更靠后、通常更旧的笔记，不受进入详情页前的 Notes Index Filter 影响。首尾缺失的相邻项保留禁用位置，以维持 footer 布局稳定。
_Avoid_: 筛选结果导航、文件顺序导航、循环导航
