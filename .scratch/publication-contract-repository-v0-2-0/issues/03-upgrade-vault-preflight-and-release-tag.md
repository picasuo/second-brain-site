...
**What to build:** Vault 的 Release Preflight 与派发配置精确采用 `@picasuo/publish-set-contract@0.2.1`，并由版本化 Vault Release Tag 触发公开部署，使每个待发布的 Vault Revision 都以明确的 `contract_version` 派发给 Site。
...
- 2026-08-07：已在 Vault 工作区完成依赖与锁文件升级（`@picasuo/publish-set-contract@0.2.1`），并将派发收据版本改为从 `pnpm install --frozen-lockfile` 后已安装包的 `publicationContractVersion` 读取。工作流仅匹配版本化 `v*` tag，会先运行只读 Release Preflight；发布脚本创建 `vX.Y.Z`。Vault 的测试与实际只读预检均通过，未写入笔记。相关 Vault 提交：`28d4f07`、`a2bd098`。
